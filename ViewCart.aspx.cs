...
public partial class ViewCart : System.Web.UI.Page
{
    ...
    protected void gvCart_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        try
        {
            ...
            Image imgDresses = (Image)row.FindControl("imgDressesImage");
            int CartId = Convert.ToInt32(imgDresses.AlternateText);
            Label LblDressesPrice = (Label)row.FindControl("LblDressesPrice");
            decimal Price = Convert.ToDecimal(LblDressesPrice.Text);
            TextBox TaxQuantity = (TextBox)row.FindControl("TxtDressesQuantity");
            int Quantity = Convert.ToInt16(TaxQuantity.Text);
            decimal ProdTotal = Quantity * Price;
            String SqlQuerry = "UPDATE Cart_Master SET Dresses_Quantity=@Dresses_Quantity,Dresses_Total=@Dresses_Total WHERE Cart_Id=@Cart_Id";
            SqlCommand Cmd = new SqlCommand();
            Cmd.CommandText = SqlQuerry;
            Cmd.Parameters.AddWithValue("@Dresses_Quantity", Quantity);
            Cmd.Parameters.AddWithValue("@Dresses_Total", ProdTotal);
    ...
    protected void gvCart_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        try
        {
            ...
            GridViewRow row = (GridViewRow)gvCart.Rows[e.RowIndex];
            Image imgDresses = (Image)row.FindControl("imgDressesImage");
            int CartId = Convert.ToInt32(imgDresses.AlternateText.ToString());
    ...
    protected void getTotal()
    {
        ...
        try
        {
            ...
            SqlQuery = "SELECT SUM(Dresses_Total) as Dresses_SubTotal FROM Cart_Master";
            SqlCommand Cmd = new SqlCommand();
            Cmd.CommandText = SqlQuery;
            Cmd.Connection = con;
            SqlDataReader dr = Cmd.ExecuteReader();
            while (dr.Read())
            {
                LblSubTotal.Text = dr["Dresses_SubTotal"].ToString();
            }
