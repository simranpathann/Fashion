...
public partial class Payment : System.Web.UI.Page
{
    ...
    protected void SubmitOrder()
    {
        try
        {
            ...
            string SqlQuerry = "INSERT INTO Order_Master(User_Id,Dresses_Id,Dresses_Name,Dresses_Image,Dresses_Quantity,Dresses_Price,Dresses_Total,Order_Date,Payment_Id,Order_Id) SELECT User_Id,Dresses_Id,Dresses_Name,Dresses_Image,Dresses_Quantity,Dresses_Price,Dresses_Total,Order_Date,Payment_Id,Order_Id FROM Cart_Master WHERE User_Id=@User_Id";
    ...
    protected void UpdateStock()
    {
        ...
        try
        {
            ...
            while (Dr.Read())
            {
                int Dresses_Id = Int32.Parse(Dr["Dresses_Id"].ToString());
                int Dresses_Quantity = Int32.Parse(Dr["Dresses_Quantity"].ToString());
                GetStock(Dresses_Id, Dresses_Quantity);
            }
        }
        catch (SqlException se)
        {
    ...
    protected void GetStock(int Dresses_Id, int Dresses_Quantity)
    {
        SqlConnection con;
        String conString, SqlQuery;
        ...
        con = new SqlConnection(conString);
        con.Open();
        SqlQuery = "SELECT Dresses_Stock FROM Dresses_Master WHERE Dresses_Id=@Dresses_Id";
        SqlCommand Cmd = con.CreateCommand();
        Cmd.CommandText = SqlQuery;
        Cmd.Parameters.Add("Dresses_Id", SqlDbType.Int).Value = Dresses_Id;
        int Dresses_Stock = Int32.Parse(Cmd.ExecuteScalar().ToString());
        Cmd.CommandText = "UPDATE Dresses_Master SET Dresses_Stock='" + (Dresses_Stock - Dresses_Quantity) + "'WHERE Dresses_Id='" + Dresses_Id + "'";
        Cmd.ExecuteNonQuery();
    }
    /**********************************************************************************************************************/
    /*********************************Order Manage*************************************************************************/
